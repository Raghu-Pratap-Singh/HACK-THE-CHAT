...
async function give(req, res) {
    try {
        ...
        const currInteraction = await interactionModel.findOneAndUpdate(
            { userA, userB },
            {
                $set: {
        ...
        if (isReply && lastMessage) {
            ...
            let [admin_tree, user_tree] = await Promise.all([
                treeModel.findOne({ userid: admin._id }),
                treeModel.findOne({ userid: user._id })
            ]);
            ...
            let up_user_strong = user_strong.filter((id) => {
                return id.toString() !== admin._id.toString();
            })
